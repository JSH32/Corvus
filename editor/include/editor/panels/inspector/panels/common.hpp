...
struct ComponentInfo<Linp::Core::Components::EntityInfoComponent> {
    ...
    static constexpr std::string_view name      = ICON_FA_CIRCLE_INFO " Entity Info"; // Using a more fitting icon
    ...
    static void draw(ComponentType& entityInfo) {
        ...
        ImGui::Checkbox("##Enabled", &entityInfo.enabled); // Use "##" to hide label but keep ID unique
        ImGui::SameLine();
...
struct ComponentInfo<Linp::Core::Components::MeshRendererComponent> {
    ...
    static void draw(Linp::Core::Components::MeshRendererComponent& renderer) {
        ...
        if (ImGui::Combo("##PrimitiveType", &currentPrimitive, primitiveNames, IM_ARRAYSIZE(primitiveNames))) {
            renderer.primitiveType = static_cast<Linp::Core::Components::PrimitiveType>(currentPrimitive);
        ...
        switch (renderer.primitiveType) {
            case Linp::Core::Components::PrimitiveType::Cube:
                needsRegeneration = ImGui::FloatEditor("Size", renderer.params.cube.size, 0.1f, 0.1f, 100.0f);
                break;

            case Linp::Core::Components::PrimitiveType::Sphere:
                needsRegeneration |= ImGui::FloatEditor("Radius", renderer.params.sphere.radius, 0.1f, 0.1f, 100.0f);
                needsRegeneration |= ImGui::IntEditor("Rings", renderer.params.sphere.rings, 1, 3, 50);
                needsRegeneration |= ImGui::IntEditor("Slices", renderer.params.sphere.slices, 1, 3, 50);
                break;

            case Linp::Core::Components::PrimitiveType::Plane:
                needsRegeneration |= ImGui::FloatEditor("Width", renderer.params.plane.width, 0.1f, 0.1f, 100.0f);
                needsRegeneration |= ImGui::FloatEditor("Length", renderer.params.plane.length, 0.1f, 0.1f, 100.0f);
                break;

            case Linp::Core::Components::PrimitiveType::Cylinder:
                needsRegeneration |= ImGui::FloatEditor("Radius", renderer.params.cylinder.radius, 0.1f, 0.0f, 100.0f);
                needsRegeneration |= ImGui::FloatEditor("Height", renderer.params.cylinder.height, 0.1f, 0.1f, 100.0f);
                needsRegeneration |= ImGui::IntEditor("Slices", renderer.params.cylinder.slices, 1, 3, 50);
                break;
        ...
        if (renderer.material) {
            ...
            float color[4] = {
                renderer.material->maps[MATERIAL_MAP_DIFFUSE].color.r / 255.0f,
        ...
        if (renderer.mesh) {
            ...
            ImGui::NextColumn();
            ImGui::Text("%d vertices, %d triangles", renderer.mesh->vertexCount, renderer.mesh->triangleCount);
            ImGui::EndColumns();
        }
    }
};
