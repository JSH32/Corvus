...
struct Material {
    ...
    void loadDefaultShader() {
        ...
        CORVUS_CORE_INFO("Loading default shader for material '{}'...");
        CORVUS_CORE_INFO("Vertex shader size: {} bytes", vert.size());
        CORVUS_CORE_INFO("Fragment shader size: {} bytes", frag.size());

        defaultShader = LoadShaderFromMemory(vert.data(), frag.data());

        if (defaultShader.id > 0) {
            defaultShaderLoaded = true;
            CORVUS_CORE_INFO("Default shader loaded successfully with ID: {}", defaultShader.id);
            ...
            CORVUS_CORE_ERROR("Failed to load default shader! Shader ID: {}", defaultShader.id);
        }
    }
