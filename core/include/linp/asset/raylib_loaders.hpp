...
public:
    raylib::Model* loadTyped(const std::string& path) override {
        PHYSFS_File* file = PHYSFS_openRead(path.c_str());
        if (!file) {
            LINP_CORE_ERROR("Failed to open OBJ: {}", path);
            return nullptr;
        }
        ...
        if (PHYSFS_readBytes(file, buf.data(), size) != size) {
            LINP_CORE_ERROR("Failed to read OBJ file: {}", path);
            PHYSFS_close(file);
            return nullptr;
        }
        PHYSFS_close(file);
        ...
        if (!warn.empty())
            LINP_CORE_WARN("TinyObj warning: {}", warn);
        if (!ok) {
            LINP_CORE_ERROR("TinyObj parse failed for {}: {}", path, err);
            return nullptr;
        }
        ...
        LINP_CORE_INFO("Loaded OBJ: {} (verts: {}, tris: {}, submeshes: {})",
                       path,
                       vertices.size() / 3,
                       indices.size() / 3,
                       meshes.size());
